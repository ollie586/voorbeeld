...
<div style="background-color: #f7f7f7;" class="flex lg:flex-row lg:justify-center mx-30 flex-wrap">
    <div class="flex justify-center flex-col bg-white border-2 border-black my-5 lg:ml-5 p-4 w-full lg:w-2/5 rounded">
        <?php
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
        } else {
        ...
        // pakt de data van de momenteel ingelogde gebruiker
            ...
            while ($data = $result->fetch_assoc()) { ?>
                <table class="w-full">
                    <tr class="bg-blue-400">
                        <th class="text-black border-2 border-black " colspan="2">Persoonlijke gegevens</th>
                    </tr>
                ...
                <!-- Modal toggle -->
                <button data-modal-toggle="gegevens-modal" class="knop font-bold lg:font-semibold text-xl lg:text-base rounded-lg px-3 lg:px-5 py-5 lg:py-1 mt-2 lg:mt-6 text-white">Bewerk gegevens</button>
    </div>
    <div class="bg-white flex flex-col items-center border-2 border-black my-5 lg:ml-5 p-4 w-full lg:w-2/5 rounded">
        ...
        if (isset($_POST['profielzoek'])) {
            ...
            ?>
<div class="flex items-center flex-col bg-white border-2 border-black my-5 lg:ml-5 py-2 lg:p-4 w-full lg:w-4/5 rounded">
    <h1 class="text-3xl font-bold">Bestellingen</h1>
    <!-- formulier voor het zoeken van bestellingen -->
    <form method="post" action="profiel.php?id=<?php echo $id ?>" class="w-full lg:w-4/5 my-4">
        <div class="relative">
            ...
            <?php if (isset($bonid)) { ?>
                <input type="search" name="bonid" value="<?php echo $bonid ?>" class="block w-5/6 p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Zoek het bonnummer">
            <?php } else { ?>
                <input type="search" name="bonid" class="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Zoek het bonnummer">
            <?php } ?>
            <style>
                .knop {
            ...
            <button type="submit" name="profielzoek" class="knop font-semibold text-white absolute right-3 top-2.5 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-2 lg:px-4 py-2">Zoek</button>
            ...
            <?php if (isset($_POST['profielzoek'])) { ?>
                <form action="../src/clear.php" method="post">
                    <input type="hidden" name="profielid" value="<?php echo $id ?>">
                    <button type="submit" name="clearprofiel" class="knop font-semibold text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-6 py-4 lg:text-lg lg:px-4 lg:py-2">Clear</button>
                </form>
            <?php } ?>
        </div>
    </form>
    <div class="flex flex-row justify-center flex-wrap">
        <?php
        $result = db()->query($sql);
        // haalt alle data die nodig is voor de bestelling
        if ($result->num_rows > 0) {
            // output data of each row
            while ($data = $result->fetch_assoc()) {
                                                                ...
                                                                <div class="p-6 text-center">
                                                                    ...
                                                                    <div class="flex justify-center space-x-2">
                                                                        ...
                                                                        <form action="../src/database.php" method="post">
                                                                            ...
                                                                            <input type="hidden" name="profiel" value="true">
                                                                            <?php if ($data['gebruikt'] == "nee") { ?>
                                                                                <button type="submit" name="verzilver" class="rounded-lg bg-green-400 px-3 py-1 lg:px-5 text-black hover:text-white hover:bg-green-800">
                                                                                    Verzilveren
                                                                                </button>
                                                                            <?php } elseif ($data['gebruikt'] == "ja") { ?>
                                                                                <button type="submit" name="verzilver" class="rounded-lg bg-green-400 px-3 py-1 lg:px-5   text-black hover:text-white hover:bg-green-800">
...
if ($result->num_rows > 0) {
    // output data of each row
    while ($data = $result->fetch_assoc()) { ?>
        <!-- Main modal  om gegevens te bewerken-->
                ...
                <div class="relative w-fit bg-white rounded-lg shadow">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        ...
                        <form class="" action="../src/database.php" method="post">
                            ...
                            <div class="flex space-x-2 lg:fl ex-row">
                                <div class=" w-4/5">
